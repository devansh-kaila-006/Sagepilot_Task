...
async def create_supervisor(supervisor: schemas.SupervisorCreate, db: AsyncSession = Depends(get_db), user: dict = Depends(get_current_user)):
...
async def list_supervisors(db: AsyncSession = Depends(get_db), user: dict = Depends(get_current_user)):
...
async def get_supervisor(id: int, db: AsyncSession = Depends(get_db), user: dict = Depends(get_current_user)):
